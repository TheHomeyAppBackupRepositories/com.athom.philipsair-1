...
(function () {
    ...
    async function getCurrentDataCoap(settings, callback) {
        ...
        let jsonStatus = null;
        coap.request(targetString + '/sys/dev/sync', 'post', Buffer.from(encodeCounter(statusCounter, 8)), { keepAlive: false })
            .then(response => {
            if (response.payload) {
    ...
    async function setValueDataCoap(key, value, settings, callback) {
        ...
        sleep(1000).then(() => {
            ...
            coap.request(targetString + '/sys/dev/control', 'post', Buffer.from(result), { keepAlive: false })
                .then(response => {
                // console.log(response);
                if (response.payload) {
                    const payload = response.payload.toString('utf-8');
